...
def extract_params_tfk_layer(layer_class) -> str:
    init_function = layer_class.__init__
    print("-------------------")
    print('class: ', layer_class.__name__)
    y = inspect.getfullargspec(init_function)
    pprint(y)
    print("args :", len(y.args))
    print("default :", len(y.defaults) if y.defaults else 0)

    print("-------------------")

    annotations_map = y.annotations
    if annotations_map == {}:
        ...
        default_values= y.defaults if y.defaults else []
        for idx, arg_name in enumerate(args_list):
            annotations_map[arg_name] = default_values[idx] if len(default_values) > idx else "NoneType"

    class_name = layer_class.__name__
    modules_path = layer_class.__module__
    annotations_list = [
        arg_name
        for arg_name in annotations_map
        if (arg_name != "self" and arg_name != "return")
    ]
    __params_class_args = [
        # convert_print_arg(arg_name, str(annotations_map[arg_name]))
        # arg_name
        # for arg_name in annotations_list
    ]
    __pytorch_class_args = [
        f'{arg_name} : "{str(annotations_map[arg_name])}",'
        for arg_name in annotations_list
    ]
    return {
        "class_name": class_name,
...
def convert_tfk_layer(layer_class) -> str:
    formatted_params = extract_params_tfk_layer(layer_class)
    pprint(formatted_params)
...
def batch_convert_tfk_layers():
    class_list = [
        getattr(layers, a) for a in dir(layers) if inspect.isclass(getattr(layers, a))
    ]
...
def convert_tfk_layer_module(module_name, layers):
    layer_params_sets = [extract_params_tfk_layer(layer) for layer in layers]
    ...
    layer_params_resets = [
        f"{keys} : {layer_params_shortened[keys]}" for keys in layer_params_shortened
    ]
...
def tfk_main():
    batch_list = batch_convert_tfk_layers()

    generate_file = open("../../app/src/node_graph/nodes/layer/tfk__generated.ts", "w+")
    generate_file.writelines(
        ["import { defineLayerNode } from './base';", "import * as zod from 'zod';"]
    )
    for layer in batch_list:
        FULL_CLASS[layer.__name__] = convert_tfk_layer(layer)

    for layer in FULL_CLASS:
        generate_file.writelines(FULL_CLASS[layer])

    generate_file.close()
    # subprocess.run(["deno", "fmt", "../../app/src/node_graph/nodes/layer/tfk__generated.ts"])

    group_by_module = {}
    for layer in batch_list:
        module_short_name = last_element_of_path(layer.__module__)
        if module_short_name =="export":
            module_short_name = "export_models"
    ...
    generate_file = open(
        "../../app/src/node_graph/nodes/layer/tfk__generated-lazy.ts", "w+"
    )
    generate_file.writelines(
        ["import { defineLazyLayerNode } from './base';", "import * as zod from 'zod';"]
    )
    for module_name, layers in group_by_module.items():
        generate_file.writelines(convert_tfk_layer_module(module_name, layers))
    generate_file.close()
    # subprocess.run( ["deno", "fmt", "../../app/src/node_graph/nodes/layer/tfk__generated-lazy.ts"] )
    ...
    # mod_file.writelines(
    #     [layer.__name__ + " as tfk__" + layer.__name__ + " ," for layer in batch_list]
    # )
    # mod_file.writelines("} from './tfk__generated';\n")

    # mod_file.writelines("export default {\n")
    # mod_file.writelines(
    #     ['tfk__'+layer.__name__+',' for layer in batch_list]
    # )
    # mod_file.writelines("\n};")
    ...
    # mod_file.writelines(
    #     [
    #         "baklava_app.editor.registerNodeType(tfk__{}, {});\n".format(
    #             layer.__name__,
    #             '{ category: "tfk/' + last_element_of_path(layer.__module__) + '" }',
    #         )
    #         for layer in batch_list
    #     ]
    # )
