...
pub fn parse_layer_stack(pairs: Pairs<Rule>) -> Result<Vec<LayerStackElm>, Error> {
    ...
    for pair in pairs {
        match pair.as_rule() {
            ...
            Rule::branch_block => {
                let tmp = parse_branch_block(pair)?;
                layer_stack.push(tmp);
            }
            _ => {}
        }
    ...
    for (pos, arg_pair) in inner_pair.clone().into_inner().enumerate() {
        match arg_pair.as_rule() {
            ...
            Rule::int_block => {
                let tmp = LayerFuncArgSet {
                    name: format!("arg:{}", pos),
                    value: LayerFuncArgValue::Int(arg_pair.as_str().parse::<i32>().unwrap()),
                };
                func_args.push(tmp);
            }
            ...
            Rule::float_block | Rule::num_block => {
                let tmp = LayerFuncArgSet {
                    name: format!("arg:{}", pos),
                    value: LayerFuncArgValue::Float(arg_pair.as_str().parse::<f32>().unwrap()),
                };
                func_args.push(tmp);
            }
...
fn parse_branch_block(pair: Pair<Rule>) -> Result<LayerStackElm, Error> {
    ...
    branch_block = parse_layer_stack(pair.into_inner())?;
